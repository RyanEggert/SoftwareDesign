...
class PyGameMouseController:
    def __init__(self,model, speed):
        self.model = model
        self.paddleSpeed = paddleSpeed  #Must be greater than or equal to 1
    
    def handle_mouse_event(self,event):
        if event.type == MOUSEMOTION:
            self.model.paddle.x = (event.pos[0] - self.model.paddle.width/2.0)*self.paddleSpeed
            if self.model.paddle.x > 590:   # width of display - (paddlewidth/2)
                self.model.paddle.x = 590
            elif self.model.paddle.x < -50: # 0 - (paddlewidth/2)    
                self.model.paddle.x = -50
...
if __name__ == '__main__':
    ...
    controller = PyGameMouseController(model, 1)
